...
const seed = async () => {
  ...
  const show1 = await Show.findByPk(2);
  const show2 = await Show.findByPk(1);
  const show3 = await Show.findByPk(7);
  const show4 = await Show.findByPk(11);
  const show5 = await Show.findByPk(5);
  const show6 = await Show.findByPk(3);
  user1.addShow(show1);
  user1.addShow(show2);
  user1.addShow(show3);
  user1.addShow(show4);
